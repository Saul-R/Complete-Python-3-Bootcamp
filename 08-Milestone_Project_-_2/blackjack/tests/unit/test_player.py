...
class TestBlackjackPlayer(unittest.TestCase):
    ...
    def test_create_player(self):
        ...
        self.assertEqual(len(my_test_player.hand.cards), 0)

    def test_get_card(self):
        my_test_player = BlackjackPlayer(name='John', starting_money=100)
        my_deck = PokerDeck()
        my_test_player.get_card(my_deck)
        self.assertEqual(len(my_test_player.hand.cards), 1)
        self.assertIsInstance(my_test_player.hand.cards[0], PokerCard)
